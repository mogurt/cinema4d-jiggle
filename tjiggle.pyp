...
class DataContainer(object):
    ...
    @property
    def strength(self):
        return self.data[SETTINGS_STRENGTH]

    @strength.setter
    def strength(self, value):
        self.data[SETTINGS_STRENGTH] = value

    @property
    def originObject(self):
        return self.data[SETTINGS_ORIGIN_OBJECT]

    @property
    def targetOffset(self):
        return self.data[SETTINGS_TARGET_OFFSET]

    @targetOffset.setter
    def targetOffset(self, value):
        self.data[SETTINGS_TARGET_OFFSET] = value

    # time

    @property
    def startTime(self):
        return self.data[SETTINGS_START_TIME]

    @startTime.setter
    def startTime(self, value):
        self.data[SETTINGS_START_TIME] = value

    # up vector
    @property
    def upVector(self):
        return self.data[SETTINGS_UP_VECTOR]

    @upVector.setter
    def upVector(self, value):
        self.data[SETTINGS_UP_VECTOR] = value

    # aim vector
    @property
    def aimVector(self):
        return self.data[SETTINGS_AIM_VECTOR]

    @aimVector.setter
    def aimVector(self, value):
        self.data[SETTINGS_AIM_VECTOR] = value
...
class Jiggle(c4d.plugins.TagData):
    ...
    def Init(self, node):
        ...
        # up vector
        data.upVector = SETTINGS_UP_VECTOR_YPLUS

        # aim vector
        data.aimVector = SETTINGS_AIM_VECTOR_ZPLUS
    ...
    def Execute(self, tag, doc, op, bt, priority, flags):
        ...
        if data.upVector == SETTINGS_UP_VECTOR_XPLUS:
            up = c4d.Vector(1.0, 0, 0) * originMatrix
        elif data.upVector == SETTINGS_UP_VECTOR_XMINUS:
            up = c4d.Vector(-1.0, 0, 0) * originMatrix
        elif data.upVector == SETTINGS_UP_VECTOR_YPLUS:
            up = c4d.Vector(0, 1.0, 0) * originMatrix
        elif data.upVector == SETTINGS_UP_VECTOR_YMINUS:
            up = c4d.Vector(0, -1.0, 0) * originMatrix
        elif data.upVector == SETTINGS_UP_VECTOR_ZPLUS:
            up = c4d.Vector(0, 0, 1.0) * originMatrix
        elif data.upVector == SETTINGS_UP_VECTOR_ZMINUS:
            up = c4d.Vector(0, 0, -1.0) * originMatrix

        side = up.Cross(aim)

        # change input order based on aim axis
        if data.aimVector == SETTINGS_AIM_VECTOR_XPLUS:
        ...
        elif data.aimVector == SETTINGS_AIM_VECTOR_XMINUS:
            jiggleMatrix = c4d.Matrix(
                originPosition,
                -aim,
                up,
        ...
        elif data.aimVector == SETTINGS_AIM_VECTOR_YPLUS:
            jiggleMatrix = c4d.Matrix(
                originPosition,
                side,
                aim,
        ...
        elif data.aimVector == SETTINGS_AIM_VECTOR_YMINUS:
            jiggleMatrix = c4d.Matrix(
                originPosition,
                side,
                -aim,
        ...
        elif data.aimVector == SETTINGS_AIM_VECTOR_ZPLUS:
            jiggleMatrix = c4d.Matrix(
                originPosition,
                side,
                up,
        ...
        elif data.aimVector == SETTINGS_AIM_VECTOR_ZMINUS:
            jiggleMatrix = c4d.Matrix(
                originPosition,
                side,
                up,
